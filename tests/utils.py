...
def cleanse_javascript(input):
    out = JS_EXCEPTION.sub('### EXCEPTION ###{linesep}\\g<exception>\\g'.format(linesep=os.linesep), input)
...
def cleanse_python(input):
    out = PYTHON_EXCEPTION.sub('### EXCEPTION ###{linesep}\\g<exception>\\g'.format(linesep=os.linesep), input)
